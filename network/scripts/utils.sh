...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
 #peer chaincode instantiate -o orderer.property-registration-network.com:7050 -C "$CHANNEL_NAME" -n regnet -l "${LANGUAGE}" -v "${VERSION}" -c '{"Args":["org.certification-network.certnet:instantiate"]}' -P "OR ('registrarMSP.member','usersMSP.member')" >&log.txt
    peer chaincode instantiate -o orderer.property-registration-network.com:7050 -C "$CHANNEL_NAME" -n regnet -l "${LANGUAGE}" -v "${VERSION}" -c '{"Args":["org.registration-network.usercontract:instantiate"]}' -P "OR ('registrarMSP.member','usersMSP.member')" >&log.txt
    res=$?
    set +x
  else
    set -x
